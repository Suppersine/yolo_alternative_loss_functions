...
class KLDloss_new(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        if self.compute == 'KLD':
            kld_pt_loss = self.KLD_compute(pred, target) #Kullback-Leibler Distance
            ...
            bd_pt_loss = self.BD_compute(pred, target) # Bhattacharrya Distance
            if self.reduction == 'none':
                bd = bd_pt_loss
            elif self.reduction == 'mean':
            ...
            gwd_pt_loss = self.GWD_compute(pred, target) # Gromov-Wasserstein Distance
            if self.reduction == 'none':
                gwd = gwd_pt_loss
            elif self.reduction == 'mean':
...
if __name__ == '__main__':
    ...
    kld_loss_n_log = KLDloss_new(alpha=1,taf=3.0,fun='log1p')
    kld_loss_n_sqrt = KLDloss_new(alpha=1,taf=3.0,fun='sqrt')
    kld_loss_o_log = KLDloss(taf=3.0,fun='log1p')
    kld_loss_o_sqrt = KLDloss(taf=3.0,fun='sqrt')
    ...
    case1 = kld_loss_n_log(target, pred)
    case2 = kld_loss_n_sqrt(target, pred)
    case3 = kld_loss_o_log(target, pred)
    case4 = kld_loss_o_sqrt(target, pred)
    #iou = KLD_angle(pred, target)  # iou(prediction, target)  shape=(num)
    print(f"{case1}\n{case2}\n{case3}\n{case4}")
